# Planning Workflow

For multi-task features, releases, or work spanning 5+ issues.

## When to Use

- Feature with multiple subtasks
- Release planning
- Work that needs epics and dependencies
- User says "plan this feature", "implement [complex thing]"

## Pattern

```
# 1. Create plan
context_plan_create
  title="Feature Name"
  content="## Overview\n\n## Goals\n\n## Success Criteria"
  status="active"

# 2. Create epics with tasks linked to plan
context_issue_create_batch
  planId="plan_..."
  issues=[
    { title: "Epic: Feature A", issueType: "epic", details: "## Implementation\n..." },
    { title: "Task 1", parentId: "$0", issueType: "task" },
    { title: "Task 2", parentId: "$0", issueType: "task" }
  ]
  dependencies=[
    { issueIndex: 2, dependsOnIndex: 1, dependencyType: "blocks" }
  ]

# 3. Present plan to user before implementing
```

## Plan Content Structure

```markdown
## Overview
[What this plan accomplishes]

## Goals
- Goal 1
- Goal 2

## Priority Order
1. Epic A (highest)
2. Epic B
3. Epic C (backlog)

## Success Criteria
- All tests pass
- No regressions
```

## Batch Creation with References

Use `$N` to reference earlier issues in the batch:

```
issues=[
  { title: "Epic: Auth", issueType: "epic" },        # $0
  { title: "Add JWT types", parentId: "$0" },        # $1, child of $0
  { title: "Add middleware", parentId: "$0" },       # $2, child of $0
  { title: "Add MCP tool", parentId: "$0" }          # $3, child of $0
]
dependencies=[
  { issueIndex: 2, dependsOnIndex: 1, dependencyType: "blocks" },  # $2 blocked by $1
  { issueIndex: 3, dependsOnIndex: 2, dependencyType: "blocks" }   # $3 blocked by $2
]
```

## Managing Plans

```
# List active plans
context_plan_list

# Get plan with linked issues
context_plan_get plan_id="plan_..."

# Mark plan complete
context_plan_update id="plan_..." status="completed"
```

## Examples

**Plan a feature:**
```
User: "implement user notifications"

→ context_plan_create title="User Notifications" content="## Overview\nReal-time notifications...\n\n## Success Criteria\n- Users receive notifications\n- Read/unread state tracked"

→ context_issue_create_batch planId="plan_..." issues=[
    { title: "Epic: Notification System", issueType: "epic" },
    { title: "Add notification model", parentId: "$0" },
    { title: "Add WebSocket handler", parentId: "$0" },
    { title: "Add notification UI", parentId: "$0" }
  ]

→ "Created plan with 1 epic and 3 tasks. Ready to implement?"
```

## Do NOT

- Skip plans for multi-task work
- Create issues without linking to plan
- Start implementing before presenting plan
- Use markdown files as primary plan source (use SaveContext)
